...
- (void)updateIndicators {
    ...
    else if (mode == MBProgressHUDMode_XTY)
    {
        // Update to indeterminate indicator
        [indicator removeFromSuperview];
        indicator = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 58,58)];
        indicator.contentMode = UIViewContentModeScaleAspectFit;
        NSMutableArray * imageArray = [[NSMutableArray alloc] init];
        for(int i = 0 ; i < 17;i ++)
        {
            [imageArray addObject:[UIImage imageNamed:[NSString stringWithFormat:@"loading1_%d",i+1]]];
        }
        
        //设置动画数组
        [(UIImageView *)indicator setAnimationImages:imageArray];
        //设置动画播放次数
